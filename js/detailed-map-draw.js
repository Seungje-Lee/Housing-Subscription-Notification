...
function districtSelect(regionCode, districtName) {
  ...
  if(localStorage.getItem("district") == null){
    selectedDistrict.push(map[regionCode] + " " + districtName);    
  } else {
    selectedDistrict = JSON.parse(localStorage.getItem("district"));
    // 이미 localStorage에 저장되어 있다면 제거하고, 없으면 넣는 과정
    let index = selectedDistrict.indexOf(map[regionCode] + " " + districtName);
    if(index !== -1) {
...
function drawMap(regionCode) {
  ...
  fetch(`mapdata/${regionCode}.json`)
    .then(res => res.json())
    .then(data => {
      let selectedDistrict = JSON.parse(localStorage.getItem("district"));
      //console.log(selectedDistrict, JSON.parse(selectedDistrict).indexOf("인제시")); //0, 1 index값 출력 가능 없으면 -1
      ...
      const layer = L.geoJSON(data, {
        ...
        onEachFeature: (feature, layer) => {
          ...
          if(selectedDistrict.indexOf(feature.properties.name) !== -1) {
